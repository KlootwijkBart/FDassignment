...
Deltaelev=[0 0 0 0 0 0 -0.7 -1.1 -1.7 -2.4 -0.3 0.1 0.4 -0.6 -1.4];
Detlatrim=[0 0 0 0 0 0 2.1 2.1 2.1 2.1 2.1 2.1 2 2 2];
...
Deltaelev=[0 0 0 0 0 0 0	-0.4	-0.9	-1.5	0.4	0.6	1 0	-0.5];
Detlatrim=[0 0 0 0 0 0 2.8	2.8	2.8	2.8	2.8	2.8	2.8 2.8	2.8];
...
mass_pass=803;    
% mass_pass=695;          %reference data (all data)
% Fuelt1=4050*0.453592;       %reference data (all data)
...
%     data file
    ...
    Cl=Lift/(0.5*rho*Veq*Veq*SurfaceWing);
    Cl2=Cl*Cl;
    
    %reynolds number range 
    reynoldslst=[reynoldslst,(chord*rho*Veq)/Dynviscosity];
...
save('C:\Users\olav_\Documents\tu-delft\``FD Assignment\matlab.dat','thrustmat','-ascii');
system('C:\Users\olav_\Documents\tu-delft\``FD Assignment\thrust(1).exe');
tdata=load('C:\Users\olav_\Documents\tu-delft\``FD Assignment\thrust.dat');
...
for i = 1:length(height)
    ...
    Cd=(T1+T2)/(0.5*rholst(i)*Veqlst(i)*Veqlst(i)*SurfaceWing);    
    
    %thrust coefficient NOTE Cd is Tc lift=drag
    Tclst=[Tclst,Cd];
    
    %Standard thrust coefficient
    Tcstlst=[Tcstlst,((T1s+T2s)/(0.5*rholst(i)*Veqlst(i)*Veqlst(i)*SurfaceWing))];
       
    DeltaeEq=Deltaelev(i)-((Cmtc/Cmd)*(((T1s+T2s)/(0.5*rholst(i)*Veqlst(i)*Veqlst(i)*SurfaceWing))-((T1+T2)/(0.5*rho*Veq*Veq*SurfaceWing))));
    
    Deleveq=[Deleveq,DeltaeEq];  
    F_nredlst=[F_nredlst,F_e(i)*((s_mass/Liftlst(i))^0.5)];
    Cdlst=[Cdlst,Cd];
end



%% Plotting all required plots
%angle of attack vs cl plot
[xlst,ylst]=line(angleoa(1:6),Cllst(1:6));
plot(angleoa(1:6),Cllst(1:6),'O',xlst,ylst)
grid on;
title('Angle of attack - Cl');
...
% plot(Cdlst(1:6),Cllst(1:6),'O') %Cl vs Cd

%Elevator trim deflection vs Equivalent aispeed
%14 to 15 is with mustafa in front
plot(Veqredlst(7:13),Deleveq(7:13),'O',Veqredlst(14:15),Deleveq(14:15),'O')
grid on;
title('Elevator trim curve');
xlabel('V_{eq} (reduced) [m/s]');
ylabel('Delta trim deflection (equivalent) [deg]')

%elevator control force vs Equivalent airspeed
%14 to 15 is with mustafa in front
plot(Veqredlst(7:13),F_nredlst(7:13),'O',Veqredlst(14:15),F_nredlst(14:15),'O')
grid on;
...
%14 to 15 is with mustafa in front
% plot(angleoa(7:13),Deleveq(7:13),'O',angleoa(14:15),Deleveq(14:15),'O')
...
oswaldlst=[];
for i = 1:6
    oswaldlst=[oswaldlst,(Cl2lst(i))/((Cdlst(i)-Cd0)*pi*SurfaceWing)];
end
oswald = mean(oswaldlst)
