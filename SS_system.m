...
%% Plot SS model response to impulse input in elevator deflection 
figure(f1)
...
N = 1701; % simulate response for 170 seconds
u_de = [-0.005; zeros(N-1,1)]; % impulse elevator input for short period and phugoid
t = linspace(0,(N-1)/10,N);
[y,t] = lsim(syss,u_de,t);

% process results from state-space model for plotting
% res_V   = [V0 + y(:,1,1)];
% res_aoa = [alpha0 + y(:,2,1)*180/pi];
% res_th  = [th0 + y(:,3,1)*180/pi];
% res_q   = y(:,4,1)*180/pi;

res_V   = [V0 + y(:,1,1)];
res_aoa = [alpha0 + y(:,2,1)];
res_th  = [th0 + y(:,3,1)];
res_q   = y(:,4,1);
...
%% Plot SS model response to impulse input in aileron deflection 
% figure(f2)
% grid on
% 
% N = 301; % simulate response for 170 seconds
% u_da = [0.025; zeros(N-1,1)]; % aileron input 
% u_dr = [zeros(N,1)];
% u = [u_da,u_dr];
% t = linspace(0,(N-1)/10,N);
% [y,t] = lsim(sysa,u,t);
